...
None
#>
function Invoke-UploadTLS {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipeline = $true, ValueFromPipelineByPropertyName = $true, Mandatory = $false)]
        [ValidateSet("ca", "cert", "key")]
        [String]
        ${Certificate},
    ...
    Process {
        'Calling method: Invoke-UploadTLS' | Write-Debug
        ...
        $LocalVarUri = '/upload/tls/{certificate}'
        if (!$Certificate) {
            throw "Error! The required parameter `Certificate` missing when calling uploadTLS."
        }
        $LocalVarUri = $LocalVarUri.replace('{certificate}', [System.Web.HTTPUtility]::UrlEncode($Certificate))

        if (!$Folder) {
            throw "Error! The required parameter `Folder` missing when calling uploadTLS."
        }
        $LocalVarFormParameters['folder'] = $Folder

        if (!$File) {
            throw "Error! The required parameter `File` missing when calling uploadTLS."
        }
